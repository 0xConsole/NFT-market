...
contract NFTBasic is ERC721Enumerable, Ownable{
    ...
    constructor(address _beneficiary) ERC721("NFTBasic", "NFTBasic") {
        _baseTokenURI = "https://somebaseuri/api/";
    ...
    function mint_NFT() public payable returns (uint256){
        require(totalSupply() <= 1005, "enough minting");

        _tokenIds.increment();
        uint256 newCharacter = _tokenIds.current();
        require(msg.value >= MINT_PRICE, "not enough");
    ...
    function honorary(uint256 _id, address owner) public onlyOwner{
        require(_id > 1005, "id is low");
